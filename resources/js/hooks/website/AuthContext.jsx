...
export function AuthProvider({ children }) {
  ...
  const refreshUser = useCallback(async () => {
    ...
    const nextRoles = normalizeRoles(data.roles ?? data.user?.roles);
    ...
    try {
      ...
      if (nextToken && nextRoles.length === 0) {
        ...
        try {
          ...
          const meRoles = normalizeRoles(me.roles ?? me.user?.roles);
  ...
  const register = useCallback(async ({ name, email, phone, password }) => {
    ...
    try {
      ...
      applyAuthState({
        token: data.token || "",
        user: data.user || null,
        roles: normalizeRoles(data.roles ?? data.user?.roles),
      });
